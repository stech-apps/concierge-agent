...
export class NativeApiService {
  ...
  logOut() {
    if (navigator.userAgent.match(/iPhone|iPad|iPod/i)) {
      ...
      var status = 0;
      try {
        status = this.util.compareVersions(
          nativeAndroidVersion,
          nativeAppVersion
        );
      } catch (err) {
        status = 0;
  ...
  startPing(period, times) {
    ...
    this.servicePointSelectors.uttParameters$
      .subscribe(uttParams => {
        if (uttParams) {
