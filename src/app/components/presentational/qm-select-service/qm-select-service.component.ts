...
export class QmSelectServiceComponent implements OnInit {
    ...
    if (this.flowType === FLOW_TYPE.CREATE_VISIT || this.flowType === FLOW_TYPE.ARRIVE_APPOINTMENT) {
      ...
      const calendarServiceSubscription = this.calendarServiceSelectors.services$.subscribe((services) => {
        ...
        if (this.serviceList !== null) {
          this.filteredServiceList = <Array<IServiceViewModel>>services;
          this.resetVistorCount(this.filteredServiceList)
          this.checkMostFrequentService();
  ...
  resetShowInfoFlags(services: Array<any>) {
    if (services && services.length > 0) {
      for (let index = 0; index < services.length; index++) {
        ...
        services[index].showMoreActions = false;
        services[index].adult = false;
        services[index].child = false;
      }
    }
  }

  resetVistorCount(services: Array<any>) {
    if (services && services.length > 0) {
      for (let index = 0; index < services.length; index++) {
        services[index].adult = 1;
        services[index].child = 0;
      }
    }
  }
  ...
  onServiceSelectByVisitorCard(selectedService, isRemove: boolean) {
    if (selectedService.adult !== 0 || selectedService.child !== 0 ) {
      this.onServiceSelect(selectedService, isRemove);
    } else {
      this.translateService.get('label.error.no.visitors').subscribe(v => {
  ...
  decreaseAdults(service) {
    if (service.adult > 0) {
      service.adult -= 1;
    } else {
      this.translateService.get('label.error.visitors.min').subscribe(v => {
        this.toastService.errorToast(v);
      });
    }
  }
